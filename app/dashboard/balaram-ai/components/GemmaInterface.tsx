...
interface GemmaInterfaceProps {
    className?: string;
}

export default function GemmaInterface({ className }: GemmaInterfaceProps) {
    ...
    useEffect(() => {
        ...
        gemmaWsRef.current = new GemmaWebSocket(
            ...
            () => {
                console.log("[Gemma] WebSocket setup complete");
    ...
    }, [isStreaming, cleanupWebSocket]);

    useEffect(() => {
        if (!isStreaming || !isWebSocketReady) return;

        console.log("[Gemma] Starting image capture interval");
    ...
    useEffect(() => {
        ...
        console.log("[Gemma] Starting audio processing setup");
    ...
    const captureAndSendImage = () => {
        if (!videoRef.current || !videoCanvasRef.current || !gemmaWsRef.current) return;
        ...
        gemmaWsRef.current.sendMediaChunk(b64Data, "image/jpeg");
    };
                    ...
                    <div className="absolute inset-0 bg-black/60 flex items-center justify-center rounded-lg backdrop-blur-sm transition-all duration-300">
                        <div className="text-center space-y-3 px-4 md:px-6 py-3 md:py-4 bg-black/40 rounded-2xl backdrop-blur-md">
                            ...
                            <p className="text-white font-semibold text-base md:text-lg">
                                {connectionStatus === 'connecting' ? 'Connecting to Gemma Model...' : 'Disconnected'}
                            </p>
