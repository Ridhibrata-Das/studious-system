...
export function playAudio(audioData: Uint8Array): Promise<void> {
  return new Promise((resolve, reject) => {
    try {
...
export function getAudioLevel(audioData: Uint8Array): number {
  try {
    ...
    const normalizedLevel = Math.min(average / 32768, 1); // Normalize to 0-1 range
